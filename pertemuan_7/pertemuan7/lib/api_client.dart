...
class ApiClient {
  ...
  // Helper untuk debugging: Print URL yang diakses
  ...
  Future<http.Response> _send(Future<http.Response> Function() req) async {
    try {
      if (_debug) print('API_CLIENT: Mengirim request...');
      final res = await req().timeout(_timeout);
      if (_debug) print('API_CLIENT: Response ${res.statusCode}');
      if (res.statusCode >= 200 && res.statusCode < 300) return res;
      throw ApiException(res.statusCode, _extractMessage(res));
    } on SocketException catch (e) {
      if (_debug) print('API_CLIENT: SocketException: $e');
      throw ApiException(0, 'Tidak ada koneksi internet.');
    } on TimeoutException {
      if (_debug) print('API_CLIENT: TimeoutException');
      throw ApiException(0, 'Server tidak merespons (timeout).');
    } catch (e) {
      if (_debug) print('API_CLIENT: Error tidak dikenal: $e');
      // Menangani ClientException atau error jaringan lainnya di Web
      if (e.toString().contains('ClientException') || e.toString().contains('Failed to fetch')) {
        throw ApiException(0, 'Koneksi error: Gagal menghubungi server.');
      }
      rethrow;
    }
  }
